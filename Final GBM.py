...
#For eroding and dilating the mask for primary noise removal 
...
def drawCentroid(vid, color_area, mask, showCentroid):
	...
	contour, _ = cv2.findContours( mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) #(source, retrieval mode, approximation method)
	...
	if l > 0 :		
		...
		if M['m00'] != 0:
			cx = int(M['m10']/M['m00']) #MiXi/Mi & MiYi/Mi
			cy = int(M['m01']/M['m00'])
			center = (cx,cy)
			if showCentroid:
...
the previous cursor position (pyp).
'''
...
# the user desires free movement of cursor, left click or right click
def chooseAction(yp, rc, bc):
	out = np.array(['move', 'false'])
	if rc[0]!=-1 and bc[0]!=-1:
		
		# if distance(yp,rc)<70 and distance(yp,bc)<70 and distance(rc,bc)<70 :
		# 	out[0] = 'drag'
		# 	out[1] = 'true'
		# 	return out
